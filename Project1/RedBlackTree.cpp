...
void RBTree::ClearRBTree(Node* node)
{
	if (node != TNill)
	{
		ClearRBTree(node->leftchild);
		ClearRBTree(node->rightchild);
		if (node == root)
		{
			root = TNill;
		}
...
void RBTree::ChangeColor(Time key, Node* Tnill, int color)
{
	Node* node = FindNode(key, Tnill);
	if (color == 4)
